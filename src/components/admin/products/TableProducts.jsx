              ...
              {currentItems?.map((product) => (
                <tr key={product._id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                    {product.name.slice(0,26)}{product.name.length>26 && '...'}
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-3">
                      <button
                        onClick={() => setIsDialog({ show: true, type: "update", data: product })}
                      ...
                      <button
                        onClick={() => setIsDialog({ show: true, type: "delete", data: product })}
      ...
      <div className="bg-gray-50 px-6 py-4 dark:bg-gray-700">
        <div className="flex flex-col items-center justify-between space-y-4 sm:flex-row sm:space-y-0">
          <div className="text-sm text-gray-500 dark:text-gray-300">
            Showing <span className="font-medium">{filteredProducts?.length ? indexOfFirstItem + 1 : 0}</span> to{" "}
            <span className="font-medium">{Math.min(indexOfLastItem, filteredProducts?.length || 0)}</span> of{" "}
            <span className="font-medium">{filteredProducts?.length || 0}</span> products
          </div>
